############################
#       Colour_Indicator_Data_FilterMapTest.py
#
#       Bore Comparison w/ Colours, 5 max, 
#       with measurement + publication type filtering in both terminal + comparison panel
#       - Latest version with marker labels now
############################

# My current main contribution to the project #

# An extension of Multi_Selection_Test.py, with the following additions or changes:
# - Changed the markers used from Folium standard to custom SVG icons so that they can be recoloured 
#   (for assigning the same colour as they are on the comparison plot). Uses JS, like previous for real-time updates.
# - Compare up to 5 bores' data at a time, currently only one measurement type & publication type at a time.
# - Choose the measurement type + publication type in terminal before generating the map, e.g. 1 = depth below ground, 1 again = field visits (for plot data, not live comparison panel).
# - Makes sure only the right data is loaded, previous versions just mixed up all the data so we had DBG = -540m or something which wasn't correct.
# - Added labels to the markers as well to see chosen measurement and publication types (set in terminal initially, these don't auto update)

# Original file (dylan-code\Multi_Selection_Test.py) used partial AI generated code for assistance with Javascript + JSON and Plotly alongside my own edits, please see full description there.

# Import all necessary libraries
import folium
import pandas as pd
import matplotlib.pyplot as plt
import io
import base64
import json
import re
from pathlib import Path

## -- Set the file paths to the data -- ##
BORES_CSV = Path("Datasets/NT-NaturalResourceMapsBoreData/Bores.csv")

GROUNDWATER_FOLDER = Path(r"Datasets/GroundwaterHeads_20250718")

MAP_FILE = "ti_tree_bores_map.html"

# -- Filter for the map -- 
# - Set to True to only show Ti Tree bores with groundwater data (using for testing atm, full mode has a lot to load - 5026 Bores)
# - Set to False to show all the bores available with our data, including ones with no available groundwater data. 
#   - Currently not working, or takes a really long time with my old hardware so can't tell if all load at once. Might need to load a different way to speed this up...
#   - Have commented out for now, and also commented out section where it is referenced and replaced with forced Ti Tree bores w/ groundwater only.
GROUNDWATER_ONLY = True

# When BOM data is implemented, can use something like RAINWATER_ONLY for rainfall as an available data measurement type? 

# Add up to 5 bores into the comparison panel graph for comparison, can add more or less but 5 seems appropriate for now.
MAX_COMPARISON_BORES = 5

# Can change the colours later but a good mix of different colours was good to test with
COMPARISON_COLOURS = [
    "#1f77b4",  # blue
    "#2ca02c",  # green
    "#d62728",  # red
    "#ff7f0e",  # orange
    "#9467bd",  # purple
]
# Can also set a select colour in a new var for salinity / chem composition bore filtering if implemented later

## -- Load the Bore Data and metadata -- ##

bores = pd.read_csv(
    BORES_CSV,
    engine="python",
    on_bad_lines="warn",
    quotechar='"',
    quoting=3
)

# Remove any whitespace or quotes from headers (strip and clean data headers)
bores.columns = bores.columns.astype(str).str.strip().str.strip('"')

required_bore_columns = {"Bore no", "Latitude", "Longitude"}

missing_columns = required_bore_columns - set(bores.columns)

# Unlikely to be missing these columns with our current set but if we implement updating from source, good to have this check in place now. 
if missing_columns:
    raise ValueError(
        "The following required columns are missing from Bores.csv: "
        f"{sorted(missing_columns)}\n"
        f"Columns found: {bores.columns.tolist()}"
    )

# Standardise the Bore IDs by converting Bores.csv's Bore no into BoreID to match the other CSVs
bores["BoreID"] = bores["Bore no"].astype(str).str.strip().str.strip('"').str.upper()

# Convert the Lat + Lon coordinates to numbers so no errors arise
bores["Latitude"] = pd.to_numeric(bores["Latitude"], errors="coerce")
bores["Longitude"] = pd.to_numeric(bores["Longitude"], errors="coerce")

# Set the zoom to encompass the Ti Tree region by default when opening the map.
ti_tree_area = (
    bores["Latitude"].between(-23, -19)
    & bores["Longitude"].between(131, 136)
)

bores_ti_tree = bores[ti_tree_area].copy()

# Keep the printed information from previous program file, still haven't incorporated it into a display on the map yet. 
print("=== Bores ===")
print("\nTotal bores from current data:", len(bores))                 # Get the total number of bores in the Bores CSV
print("\nBores in the Ti Tree area:", len(bores_ti_tree))             # Get the number of bores only in the Ti Tree area 
print("\nSample BoreIDs:", bores["BoreID"].dropna().unique()[:5])     # Print 5 of the BoreIDs to the terminal, mainly used for testing data was being passed correctly at each stage. Remove later?

## -- Load the Groundwater CSVs from Datasets/GroundwaterHeads_20250718 -- ###
# As before in original file, but modified for more accuracy. Now looking for keywords (the Bore identifier (@RN(XXXXXX))). 

# Set search filter / pattern to find the BoreID by looking for @RN(XXXXXX)
# e.g. file name like DataSetExport-Depth Below Ground.Field Visits@RN005626-20250718020600.csv = @RN005626
bore_id_pattern = re.compile(r"@(RN\d{6})-")

# Load data into the dataframe, structure is groundwater_data["RN005626"] = DataFrame
groundwater_data = {}

# Don't need to alert user for successful folder find every time now, just alert if folder not found.
if not GROUNDWATER_FOLDER.exists():
    raise FileNotFoundError(
        "Groundwater folder was not found at the expected file path: \n"
        f"{GROUNDWATER_FOLDER.resolve()}"
    )

# There are 64 CSVs available, which is also counting the non-LocationExport subfolder CSVs too - maybe remove these? 
groundwater_files = list(GROUNDWATER_FOLDER.rglob("*.csv"))

print("\n=== Groundwater ===")
print("\nGroundwater folder:", GROUNDWATER_FOLDER.resolve())
print("\nCSV files found recursively:", len(groundwater_files))

for file_path in groundwater_files:
    match = bore_id_pattern.search(file_path.stem)

    if not match:
        print("Skipping file because no BoreID was found:", file_path.name)
        continue

    bore_id = match.group(1) # e.g. "RN005626"

    # Filter the file name, looking for key part separators '-' and '.' which separate measurement & publish types & BoreID
    filename_part = file_path.stem.split("@", 1)[0]

    if "-" not in filename_part:
        print("Skipping file because dataset part could not be parsed:", file_path.name)
        continue

    dataset_part = filename_part.split("-", 1)[1]

    if "." in dataset_part:
        measurement_type, publication_type = (
            dataset_part.split(".", 1)
        )
    else:
        measurement_type = dataset_part
        publication_type = "Unknown"

    measurement_key = (
        measurement_type
        .strip()
        #.lower()           # Result is still Ahd instead of AHD regardless but we can live with that for now.
        .replace(" ", "_")
    )

    publication_key = (
        publication_type
        .strip()
        .lower()
        .replace(" ", "_")
    )

# Skip 2 rows, using row 3 as the header. Load the data.
    try:
        groundwater_df = pd.read_csv(
            file_path,
            skiprows=2,
            header=0,
            engine="python"
        )

    # Display error for unreadable / unfindable data CSVs. 
    except Exception as error:
        print(
            f"\nCould not read {file_path.name}: {error}"
        )
        continue

    # Clean the groundwater data column names
    groundwater_df.columns = groundwater_df.columns.astype(str).str.strip().str.strip('"')

    # Detect timestamp and value columns dynamically
    timestamp_column = None
    value_column = None

    for column in groundwater_df.columns:
        column_lower = column.lower()

        # Find the timestamp column - contains 'timestamp' but not 'event' 
        if ("timestamp" in column_lower and "event" not in column_lower):
            timestamp_column = column

        # Find the value column - contains 'value' and 'm'
        if "value" in column_lower:
            value_column = column

    # If there's no columns that match, dataset might not be valid.
    if timestamp_column is None or value_column is None:
        print(f"\nSkipping file because timestamp/value columns were not found: {file_path.name}")
        print("\nColumns found:", groundwater_df.columns.tolist())
        continue

# Convert data types
    groundwater_df["Timestamp"] = pd.to_datetime(groundwater_df[timestamp_column], errors="coerce", dayfirst=False)

    groundwater_df["Value (m)"] = pd.to_numeric(groundwater_df[value_column], errors="coerce")

    groundwater_df = (groundwater_df[["Timestamp", "Value (m)"]].dropna(subset=["Timestamp", "Value (m)"]).sort_values("Timestamp"))

    # Skipped files include the 4 CSVs in the Groundwater folder not under LocationExport subfolders, no BoreID column found.
    if groundwater_df.empty:
        print("\nSkipping file because it contains no usable records:", file_path.name)
        continue

    groundwater_data.setdefault(bore_id, {})
    groundwater_data[bore_id].setdefault(measurement_key, {})
    groundwater_data[bore_id][measurement_key][publication_key] = groundwater_df

print("\nGroundwater time series loaded:", len(groundwater_data))

if groundwater_data:
    print("\nSample groundwater BoreIDs:", list(groundwater_data.keys())[:5])

# Print 5 sample BoreIDs and available measurement + publish types to the terminal 
# (mainly for testing correct data CSVs could be found & not overwritten like earlier programs I coded)
# These will be DBG + WE, with primarily field_visits but sometimes publish as well.
for bore_id, categories in list(groundwater_data.items())[:5]:
    print(f"\n{bore_id}")
    for category, publication_types in categories.items():
        print(f"  {category}")
        for publication_type in publication_types:
            print(f"    {publication_type}")

## -- Select the Measurement + Publication Types -- ##

# Get the available categories from the data to give as options for the user to select later
available_categories = sorted({
    category
    for bore_data in groundwater_data.values()
    for category in bore_data
})

if not available_categories:
    raise RuntimeError(
        "No measurement categories were found in the groundwater data."
    )

# Ask the user for input, choose an available measurement category (DBG or WE)
print("\nAvailable measurement categories:")
for number, category in enumerate(available_categories, start=1):
    print(f"{number}: {category}")

category_choice = input("Select a measurement category (number): ").strip()

try:
    selected_category = available_categories[int(category_choice) - 1]
except (ValueError, IndexError):
    raise ValueError("Invalid measurement category selection.")

# Get the available publication types from the data to give as options for the user to select later 
available_publication_types = sorted({
    publication_type
    for bore_data in groundwater_data.values()
    if selected_category in bore_data
    for publication_type in bore_data[selected_category]
})

if not available_publication_types:
    raise RuntimeError(
        f"No publication types found for category '{selected_category}'."
    )

# Ask the user for input, choose an available publication category (Field Visits or Publish)
print("\nAvailable publication types:")
for number, publication_type in enumerate(available_publication_types, start=1):
    print(f"{number}: {publication_type}")

publication_choice = input("Select a publication type (number): ").strip()

try:
    selected_publication_type = available_publication_types[int(publication_choice) - 1]
except (ValueError, IndexError):
    raise ValueError(
        "Invalid publication type selection."
    )

print("\nSelected:", selected_category, selected_publication_type)

# Build a simple dictionary for mapping
selected_groundwater_data = {}

for bore_id, bore_data in groundwater_data.items():
    if selected_category not in bore_data:
        continue
    if selected_publication_type not in bore_data[selected_category]:
        continue

    selected_groundwater_data[bore_id] = bore_data[selected_category][selected_publication_type]

print("BoreIDs with selected dataset:", len(selected_groundwater_data))

## -- Select the bores to be displayed -- ##
# Currently only have Groundwater data, but this section was for if Rainfall was implemented, while I remembered
if GROUNDWATER_ONLY:
    available_bore_ids = set(groundwater_data.keys())
    bores_to_map = bores_ti_tree[bores_ti_tree["BoreID"].isin(available_bore_ids)].copy()

# elif RAINWATER_ONLY:
    # set available bores to rainwater data, map the rainwater data only, etc

else:
    bores_to_map = bores_ti_tree.copy()

## Removed BoreID overlap portion for now. ##

bores_to_map = bores_to_map.dropna(subset=["Latitude", "Longitude"])

bore_name_map = (
    bores_to_map[["BoreID", "Name"]]
    .drop_duplicates("BoreID")
    .set_index("BoreID")["Name"]
    .to_dict()
)

# Ensure all keys and values are strings and safe for JSON
bore_name_map = {
    str(k): (str(v) if pd.notna(v) else "")
    for k, v in bore_name_map.items()
}

print("\n=== Map Data ===")
print("\nBores selected for mapping:", len(bores_to_map))
print("\nBoreIDs selected:", bores_to_map["BoreID"].tolist())

## -- Create the bore plots on the map marker popup displays -- ## 

def plot_bore_popup(bore_id, bore_row, groundwater_data, measurement_label, publication_label):
    # This will create an individual bore graph displayed, as well as an 'add/remove comparison' button 
    # to add the data plot to the comparison panel. 
    # The BoreID is stored in a separate variable data-bore-id on the click of the button because using 'onclick' didn't work correctly. 
    
    figure, axis = plt.subplots(figsize=(8, 4))

    # Groundwater time series plot
    if bore_id in groundwater_data:
        groundwater_df = groundwater_data[bore_id]

        axis.plot(
            groundwater_df["Timestamp"],
            groundwater_df["Value (m)"],
            color="blue",
            marker="o",
            markersize=2,
            linewidth=1,
            label="Groundwater value"
        )

        axis.set_xlabel("Date")
        axis.set_ylabel("Value (m)")
        axis.set_title(f"Bore {bore_id}")
        axis.grid(True, alpha=0.3)

        # Comment out for Water Elevation data, Uncomment for Depth Below Ground (will need to automate this in future updates)
        #axis.invert_yaxis()

    # If data is unavailable and marker is still placed, add a placeholder to the marker content window
    else:
        axis.text(
            0.5,
            0.5,
            "There is no groundwater data available",
            transform=axis.transAxes,
            ha="center",
            va="center"
        )

        axis.set_title(f"Bore {bore_id}")

    figure.autofmt_xdate()
    figure.tight_layout()

    image_buffer = io.BytesIO()
    
    # Save the plot as a png image in a Base64 buffer
    figure.savefig(image_buffer, format="png", bbox_inches="tight")

    image_buffer.seek(0)

    image_base64 = base64.b64encode(image_buffer.read()).decode()

    plt.close(figure)

    bore_name = bore_row.get("Name", "")

    if pd.isna(bore_name):
        bore_name = ""

    # Escape (convert data into a safe format) the metadata displayed as HTML
    bore_name = (
        str(bore_name)
        .replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
    )

    # Escape the BoreID for use inside an HTML attribute.
    bore_id_attribute = (
        str(bore_id)
        .replace("&", "&amp;")
        .replace('"', "&quot;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
    )

    # Escape the measurement label data
    measurement_label = (
        str(measurement_label)
        .replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
    )

    # Escape the publication label data
    publication_label = (
        str(publication_label)
        .replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
    )

    # Prepare the HTML for the map for this portion
    # Consists of Bore ID, Bore Name, the plotted data, and the Add/Remove Comparison button.     
    return f"""
    <div style="
        font-family: Arial, sans-serif;
        font-size: 12px;
        min-width: 290px;
    ">
        <div>
            <b>Bore ID:</b> {bore_id_attribute}<br>
            <b>Name:</b> {bore_name}<br>
            <b>Measurement type:</b> {measurement_label}<br>
            <b>Data type:</b> {publication_label}
        </div>

        <img
            src="data:image/png;base64,{image_base64}"
            style="
                max-width: 280px;
                display: block;
                margin-top: 6px;
            "
        >

        <button
            type="button"
            class="comparison-toggle-button"
            data-bore-id="{bore_id_attribute}"
            style="
                margin-top: 8px;
                padding: 5px 8px;
                cursor: pointer;
            "
        >
            Add/remove comparison
        </button>
    </div>
    """

## -- Create the Folium Map -- ##
map_object = folium.Map(location=[-21.5, 134.5], zoom_start=7, control_scale=True)

## -- Embed the comparison data as JSON -- ##
# Need to update the markers in real time for this feature to work so research led me to JSON + JS. 

comparison_data = {}

for bore_id in bores_to_map["BoreID"].dropna().unique():
    bore_id = str(bore_id)

    if bore_id not in groundwater_data:
        continue

    bore_datasets = groundwater_data[bore_id]
    comparison_data[bore_id] = {}

    for measurement_key, publications in bore_datasets.items():
        comparison_data[bore_id][measurement_key] = {}

        for publication_key, df in publications.items():
            df = df.copy()
            df["Timestamp"] = pd.to_datetime(
                df["Timestamp"], errors="coerce"
            )
            df["Value (m)"] = pd.to_numeric(
                df["Value (m)"], errors="coerce"
            )
            df = df.dropna(subset=["Timestamp", "Value (m)"])
            if df.empty:
                continue
            df = df.sort_values("Timestamp")

            comparison_data[bore_id][measurement_key][publication_key] = {
                "timestamps": (
                    df["Timestamp"]
                    .dt.strftime("%Y-%m-%d %H:%M:%S")
                    .tolist()
                ),
                "values": (
                    df["Value (m)"]
                    .astype(float)
                    .tolist()
                )
            }

comparison_json = json.dumps(comparison_data, allow_nan=False)
bore_name_json = json.dumps(bore_name_map)

print("\nComparison series embedded:", len(comparison_data))

print("Embedded BoreIDs:", list(comparison_data.keys()))

## -- Add the comparison panel to the map, on the right-hand-side -- ##
# Comparison panel has a title 'Bore Comparison', list of selected bores by bore ID, a warning if too many 
# bores have been selected (reset bores button is available for this), and the comparison plot. Drop down selectors for 
# data types added now too.

# <!-- 
# Note to team: This panel overrides previously selected data format from terminal selection, without updating the markers (currently). 
# Markers will only have what they were initialised with when the map was generated. 
# There are also some bugs when selecting bores for comparison, resulting in broken graph so just clear selection and try again or close & reopen map - will need to fix eventually. 
# Default is DBG + FV, problem seems to happen with WE & or Publish data (is also quite laggy too - address this also, may just be my hardware).
# -->
# - Graph on the marker will also be different to the comparison plot for opposing types (measurement + publication) because of "#axis.invert_yaxis()" on line 371
# Remove the div element from line 617 to line 620 to remove this info from display on the comp panel in browser.

comparison_panel = """
<div id="comparison-panel"
     style="
       position: fixed;
       top: 10px;
       right: 10px;
       width: 445px;
       height: 525px;
       background-color: white;
       border: 2px solid #666;
       border-radius: 6px;
       z-index: 9999;
       padding: 8px;
       box-shadow: 0 2px 8px rgba(0,0,0,0.3);
       font-family: Arial, sans-serif;
     ">

    <div style="font-weight: bold; margin-bottom: 4px;">
        Bore Comparison
    </div>
    
    <div style="font-weight: normal; font-style: italic; margin-bottom: 4px;">
            ** Currently no type v type comparisons, only group comparison for active type - drop down menu settings selected will apply to all currently selected bores **<br>
            ** Will need to address this later, and occasional graph view bug when changing to Publish type. **
    </div>
    
    <div style="font-size: 12px; margin-bottom: 6px;">
        <div style="margin-bottom: 4px;">
            <label for="measurement-select">Measurement type:</label>
            <select id="measurement-select"
                    style="width: 100%; margin-top: 2px;">
                <!-- options filled by JS -->
            </select>
        </div>

        <div>
            <label for="publication-select">Data type:</label>
            <select id="publication-select"
                    style="width: 100%; margin-top: 2px;">
                <!-- options filled by JS -->
            </select>
        </div>
    </div>

    <div id="selected-bores" style="font-size: 12px; margin-bottom: 4px;">
        Selected bores: none
    </div>

    <div id="comparison-warning"
         style="color: #b00020; font-size: 12px; min-height: 18px;">
    </div>

    <button id="clear-comparison"
            type="button"
            style="margin-bottom: 5px; cursor: pointer;">
        Clear selection
    </button>

    <div id="comparison-plot"
         style="width: 100%; height: 330px;">
    </div>
</div>
"""

# Embed the comparison panel within the generated folium map.
map_object.get_root().html.add_child(folium.Element(comparison_panel))

## -- Create the bore icons -- ##
# - Have to do it this way by making an SVG pin icon so we can customise the colour
# - This allows colour coordination, so the colour of the BoreID on the comp plot matches the bore marker on the map.

def make_bore_icon(bore_id, colour="#3388ff"):
    bore_id_html = (
        str(bore_id)
        .replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
    )

    # Style the SVG pin, add configurable fill colour (uses blue by default)
    svg_pin = f"""
    <svg width="22" height="32" viewBox="0 0 22 32"
         xmlns="http://www.w3.org/2000/svg">
        <path
            d="M11 0C4.9 0 0 4.9 0 11c0 7.3 11 21 11 21s11-13.7 11-21C22 4.9 17.1 0 11 0z"
            fill="{colour}"
            stroke="white"
            stroke-width="2"
        />
        <circle cx="11" cy="11" r="5" fill="white" />
    </svg>
    """

    # Render the icon in html using a folium div element
    return folium.DivIcon(
        html=f"""
        <div
            class="bore-marker"
            data-bore-id="{bore_id_html}"
            style="
                width: 22px;
                height: 32px;
            "
            title="{bore_id_html}"
        >
            {svg_pin}
        </div>
        """,
        icon_size=(22, 32),
        icon_anchor=(11, 16),
    )

## -- Add the markers to the map -- ##

measurement_label = selected_category.replace("_", " ").title()
publication_label = selected_publication_type.replace("_", " ").title()

for _, row in bores_to_map.iterrows():
    bore_id = str(row["BoreID"])
    latitude = float(row["Latitude"])
    longitude = float(row["Longitude"])

    popup_html = plot_bore_popup(
        bore_id,
        row,
        selected_groundwater_data,
        measurement_label,
        publication_label
    )

    folium.Marker(
        location=[latitude, longitude],
        icon=make_bore_icon(bore_id),
        popup=folium.Popup(popup_html, max_width=350),
        tooltip=f"Open bore {bore_id}"
    ).add_to(map_object)

## -- Add Plotly.js -- ##
# Plotly is needed for the real-time rendering of the map and updates aspect.
# Folium and Plotly both use HTML + JS

plotly_script = """
<script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
"""

map_object.get_root().header.add_child(folium.Element(plotly_script))

## -- Add the comparison Javascript -- ##
# Mostly AI Generated JS but many variables and text were adapted by me for use with this python script as needed. See earlier note for more info.
# Now with more additions like colour variations for markers, additional labels for the markers, and more.  

comparison_javascript = f"""
<script>
const comparisonData = {comparison_json};
const boreNameMap = {bore_name_json};
const selectedBores = [];
const MAX_BORES = {MAX_COMPARISON_BORES};
const comparisonColours = [
    "#1f77b4",
    "#2ca02c",
    "#d62728",
    "#ff7f0e",
    "#9467bd"
];

let activeMeasurement = null;
let activePublication = null;


function formatLabel(key) {{
    return key
        .replace(/_/g, " ")
        .replace(/\\b\\w/g, function(c) {{
            return c.toUpperCase();
        }});
}}

function discoverDatasetOptions() {{
    const measurements = new Set();
    const publications = new Set();

    Object.values(comparisonData).forEach(function(boreData) {{
        Object.keys(boreData).forEach(function(measurement) {{
            measurements.add(measurement);
            Object.keys(boreData[measurement]).forEach(function(pub) {{
                publications.add(pub);
            }});
        }});
    }});

    const allMeasurements = Array.from(measurements).sort();
    const allPublications = Array.from(publications).sort();

    const measurementSelect =
        document.getElementById("measurement-select");
    const publicationSelect =
        document.getElementById("publication-select");

    measurementSelect.innerHTML = "";
    publicationSelect.innerHTML = "";

    allMeasurements.forEach(function(m) {{
        const option = document.createElement("option");
        option.value = m;
        option.textContent = formatLabel(m);
        measurementSelect.appendChild(option);
    }});

    allPublications.forEach(function(p) {{
        const option = document.createElement("option");
        option.value = p;
        option.textContent = formatLabel(p);
        publicationSelect.appendChild(option);
    }});

    if (allMeasurements.length > 0) {{
        activeMeasurement = allMeasurements[0];
        measurementSelect.value = activeMeasurement;
    }}

    if (allPublications.length > 0) {{
        activePublication = allPublications[0];
        publicationSelect.value = activePublication;
    }}

    measurementSelect.addEventListener("change", function() {{
        activeMeasurement = measurementSelect.value;
        redrawComparisonPlot();
    }});

    publicationSelect.addEventListener("change", function() {{
        activePublication = publicationSelect.value;
        redrawComparisonPlot();
    }});
}}

function updateSelectedBoresText() {{
    const selectedElement =
        document.getElementById("selected-bores");

    if (!selectedElement) {{
        return;
    }}

    if (selectedBores.length === 0) {{
        selectedElement.textContent =
            "Selected bores: none";
    }} else {{
        const parts = selectedBores.map(function(boreId) {{
            const name = boreNameMap[boreId] || "";
            if (name === "") {{
                return boreId;
            }}
            return boreId + " (" + name + ")";
        }});

        selectedElement.textContent =
            "Selected bores: " + parts.join(", ");
    }}
}}

function showComparisonWarning(message) {{
    const warningElement =
        document.getElementById("comparison-warning");

    if (warningElement) {{
        warningElement.textContent = message;
    }}
}}

function redrawComparisonPlot() {{
    const plotElement =
        document.getElementById("comparison-plot");

    if (!plotElement) {{
        console.warn(
            "The comparison plot element was not found."
        );
        return;
    }}

    if (typeof Plotly === "undefined") {{
        console.error(
            "Plotly failed to load."
        );
        return;
    }}

    const traces = [];

    selectedBores.forEach(function(boreId, index) {{
        const boreData = comparisonData[boreId];

        if (
            !boreData ||
            !boreData[activeMeasurement] ||
            !boreData[activeMeasurement][activePublication]
        ) {{
            return;
        }}

        const series =
            boreData[activeMeasurement][activePublication];

        traces.push({{
            x: series.timestamps,
            y: series.values,
            type: "scatter",
            mode: "lines+markers",
            name: boreId,
            connectgaps: false,
            line: {{
                color: comparisonColours[index],
                width: 2
            }},
            marker: {{
                color: comparisonColours[index],
                size: 6
            }}
        }});
    }});

    const measurementTitle = formatLabel(activeMeasurement);
    const publicationTitle = formatLabel(activePublication);

    const layout = {{
        title: {{
            text: "Comparison Graph of " +
                measurementTitle + " (" + publicationTitle + ")",
            font: {{ size: 14 }}
        }},
        xaxis: {{
            title: {{
                text: "Date",
                font: {{ size: 12 }}
            }},
            tickfont: {{ size: 11 }}
        }},
        yaxis: {{
            title: {{
                text: measurementTitle + " (m)",
                font: {{ size: 12 }}
            }},
            tickfont: {{ size: 11 }}
        }},
        margin: {{
            l: 60,
            r: 20,
            t: 70,
            b: 55
        }},
        legend: {{
            orientation: "h",
            font: {{ size: 11 }}
        }},
        hovermode: "x unified",
        uirevision: "keep-view"
    }};

    Plotly.react(
        "comparison-plot",
        traces,
        layout,
        {{
            responsive: true,
            displaylogo: false
        }}
    );

    updateAllMarkerColours();
    updateSelectedBoresText();
}}

function toggleBore(boreId) {{
    const existingIndex =
        selectedBores.indexOf(boreId);

    if (existingIndex !== -1) {{
        selectedBores.splice(existingIndex, 1);
        redrawComparisonPlot();
        return;
    }}

    if (selectedBores.length >= MAX_BORES) {{
        showComparisonWarning(
            "Maximum of " + MAX_BORES + " bores reached. Please remove at least one to select another, or start again by using the reset button below."
        );
        return;
    }}

    if (!comparisonData[boreId]) {{
        showComparisonWarning(
            "No comparison data is currently available for " + boreId + "."
        );
        return;
    }}

    selectedBores.push(boreId);
    redrawComparisonPlot();
}}

function setupComparisonButtonHandler() {{
    document.addEventListener(
        "click",
        function(event) {{
            const button = event.target.closest(
                ".comparison-toggle-button"
            );

            if (!button) {{
                return;
            }}

            const boreId =
                button.getAttribute("data-bore-id");

            if (!boreId) {{
                console.warn(
                    "The comparison button has no BoreID attributed."
                );
                return;
            }}

            toggleBore(boreId);
        }}
    );
}}

function setBoreMarkerColour(boreId, colour) {{
    const markerElements = document.querySelectorAll(
        ".bore-marker"
    );

    markerElements.forEach(function(markerElement) {{
        if (
            markerElement.getAttribute("data-bore-id")
            === boreId
        ) {{
            const svg = markerElement.querySelector("svg");
            const path = markerElement.querySelector("path");

            if (svg) {{
                svg.style.transform = "scale(1.2)";
                svg.style.transformOrigin = "bottom center";
            }}

            if (path) {{
                path.setAttribute("fill", colour);
            }}
        }}
    }});
}}

function resetBoreMarkerColour(boreId) {{
    const markerElements = document.querySelectorAll(
        ".bore-marker"
    );

    markerElements.forEach(function(markerElement) {{
        if (
            markerElement.getAttribute("data-bore-id")
            === boreId
        ) {{
            const svg = markerElement.querySelector("svg");
            const path = markerElement.querySelector("path");

            if (svg) {{
                svg.style.transform = "scale(1)";
            }}

            if (path) {{
                path.setAttribute("fill", "#3388ff");
            }}
        }}
    }});
}}

function updateAllMarkerColours() {{
    // Reset all markers to default
    const markerElements = document.querySelectorAll(
        ".bore-marker"
    );

    markerElements.forEach(function(markerElement) {{
        const svg = markerElement.querySelector("svg");
        const path = markerElement.querySelector("path");

        if (svg) {{
            svg.style.transform = "scale(1)";
        }}

        if (path) {{
            path.setAttribute("fill", "#3388ff");
        }}
    }});

    // Apply colours to selected bores
    selectedBores.forEach(function(boreId, index) {{
        const colour = comparisonColours[index];
        setBoreMarkerColour(boreId, colour);
    }});
}}

document.addEventListener(
    "DOMContentLoaded",
    function() {{
        discoverDatasetOptions();
        setupComparisonButtonHandler();
        redrawComparisonPlot();

        const clearButton =
            document.getElementById("clear-comparison");

        if (clearButton) {{
            clearButton.addEventListener(
                "click",
                function() {{
                    selectedBores.length = 0;
                    showComparisonWarning("");
                    redrawComparisonPlot();
                }}
            );
        }}
    }}
);
</script>
"""

map_object.get_root().html.add_child(folium.Element(comparison_javascript))

## -- Update the map & create the HTML file -- ##
# Map File name is 'ti_tree_bores.html' as set earlier

map_object.save(MAP_FILE)

print(f"\nMap saved as: {MAP_FILE}")

print("Open the map HTML file using the integrated browser (right click > Open in Integrated Browser), then click on a marker.\n")
